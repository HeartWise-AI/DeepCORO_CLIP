...
def compute_classification_metrics(
    ...
    if head_structure[head_name] == 1:
        if compute_cli:                    
            ...
            metrics[f"{mode}/{head_name}_best_threshold"] = best_threshold_fn(
                p=all_preds.flatten(), 
                t=all_targets.flatten()
            )
            metrics[f"{mode}/{head_name}_accuracy"] = accuracy_at_threshold_fn(
                threshold=metrics[f"{mode}/{head_name}_best_threshold"]
            )(p=all_preds.flatten(), t=all_targets.flatten())
            metrics[f"{mode}/{head_name}_f1_score"] = f1_at_threshold_fn(
                threshold=metrics[f"{mode}/{head_name}_best_threshold"]
            )(p=all_preds.flatten(), t=all_targets.flatten())
