...
@RunnerRegistry.register("DeepCORO_clip_simple")
@RunnerRegistry.register("DeepCORO_clip_simple_test")
class VideoContrastiveLearningRunnerSimple:
    """
    ...
    def __init__(
        ...
        if getattr(self.config, "gradient_accumulation_steps", 1) != 1:
            if self.config.is_ref_device:
                print(
                    "[RunnerSimple] gradient_accumulation_steps>1 is no longer supported; "
    ...
    def _train_step(
        ...
        if not torch.isfinite(loss):
            diagnostics = {
                "step": self.step,
                "video_norm": float(video_emb.detach().float().norm().item()),
                "text_norm": float(text_emb.detach().float().norm().item()),
            }
