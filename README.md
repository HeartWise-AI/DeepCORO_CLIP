...
To optimize data loading performance, you can adjust the `num_workers` parameter in the configuration file. This parameter controls the number of subprocesses used for data loading. Increasing the number of workers can significantly speed up data loading, especially for large datasets.

1. **Configuration File**:

In the `config/default_config.yaml` file, set the `num_workers` parameter under the `Data parameters` section:
...
You can also override the `num_workers` parameter from the command line:

```bash
python scripts/train_model.py --config config/default_config.yaml --num-workers 8
```

Increasing the `num_workers` value can improve data loading performance, but it may also increase memory usage. Experiment with different values to find the optimal setting for your system.
